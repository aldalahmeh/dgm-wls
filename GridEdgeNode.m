...
classdef GridEdgeNode < handle
    ...
    methods
        ...
        function [stateOut, convIter] = computeSingleStateEst(obj, iSim, busMeasMapping, Options, q, maxIter, tol, alpha)
            ...
            %     stateOut = xCurrent;
            % else
            %     % Send completed run index to DataQueue for progress update
            %     send(q, iSim);
            % end
            stateOut = xCurrent;
            send(q, iSim);
        ...
        %     obj.simsCompleted = 0; % Reset the counter before starting
        %     q = parallel.pool.DataQueue;
        %     afterEach(q, @(data) obj.updateMCProgress(data, nSim));
        % 
        %     % Decide pool size
        ...
        %         delete(gcp('nocreate'));
        %     else
        %         % Force exactly 20 workers
        %         desiredWorkers = 20;
        %         % desiredWorkers = 3;
        %         % Close any existing pool first (prevents mismatch)
        %         delete(gcp('nocreate'));
        %         % Start a pool with the requested number
        %         pool = parpool(desiredWorkers);
        %         % Confirm actual number of workers (profile or cluster may limit)
        %         numWorkers = pool.NumWorkers;
        %         fprintf('Starting Monte Carlo simulation (%d runs) on %d workers...\n', nSim, numWorkers);
        % 
        %         % Optional: ensure cleanup when function exits or errors
        %         % cleanupPool = onCleanup(@() delete(gcp('nocreate')));
        %     end
        % 
        % 
        %     % Parallel loop
        ...
        %         wlsConverged = false;
        % 
        %         % Clear estimate at begining of each simulation iteration
        %         xCurrent = [];
        % 
        %         % Prepare measurements
        %         zTable = obj.prepareSensorMeas(busMeasMapping, iSim);
        ...
        %             %     J = r.Residuals' * W_iter * r.Residuals;
        %             %     fprintf('[Iter %d]: Residual norm squared is %2.4f\n',iter, J);
        %             % end
        % 
        ...
        % 
        %             % if (Options.DebugMode == true)
        %             %     fprintf('WLS did not converge within %d iterations for MC run %d.\n', maxIter, iSim);
        %             %     fprintf('Max estimate update is %1.4f\n', max_update);
        %             % end
        ...
        function estData = estCorruptData(obj, data, Options)
            ...
            for k = 1:length(corruptRows)
                ...
                % ---------------------------------------------------------
                if strcmpi(Options.dataImpute, 'MAP')
        ...
        % function W_iter = applyRobustWeightsOld(~, W_base, raw_res, Options)
